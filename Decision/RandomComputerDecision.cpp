	...
	const Gladiators& arena)const
{
	if (!decidingUnit.m_magicBook.canCastAnySpell())
		return nullptr;
	index magicToCastIndex = randomNumber((int)decidingUnit.m_magicBook.size() - 1);
	while (!decidingUnit.isEnoughManaFor(decidingUnit.m_magicBook[magicToCastIndex]))
		magicToCastIndex = randomNumber((int)decidingUnit.m_magicBook.size() - 1);
...
{
	index unitToCast = randomNumber((int)arena.size() - 1);
	while (isWrongSpellToCast(decidingUnit, *arena[unitToCast], magicToCast))
		unitToCast = randomNumber((int)arena.size() - 1);
